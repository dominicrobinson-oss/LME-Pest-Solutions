    ...
    <PublicShell>
      <section className="bg-[#07131d] text-white">
        <div className="container-lme grid gap-10 py-14 lg:grid-cols-[1fr_420px] lg:py-20">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.16em] text-[var(--primary-green)]">{eyebrow}</p>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-3">
              {["Tracked enquiry", "Clear treatment notes", "Follow-up where needed"].map((item) => (
                <div className="flex items-center gap-2 text-sm font-bold text-slate-200" key={item}>
                  <CheckCircle2 className="text-[var(--primary-green)]" size={18} />
                  {item}
                </div>
              ))}
            </div>
      ...
      <section className="bg-slate-50 py-14">
        <div className="container-lme grid gap-6 lg:grid-cols-[1fr_360px]">
          <div>
            <p className="text-sm font-black uppercase tracking-[0.16em] text-[var(--primary-green)]">No invented claims</p>
  ...
  return (
    <section className="card p-5">
      <div className="flex items-center gap-3">
        <span className="grid size-11 place-items-center rounded-lg bg-lime-50 text-[var(--primary-green)]"><Icon size={22} /></span>
        <h2 className="text-xl font-black">{title}</h2>
      </div>
      <div className="mt-4 grid gap-3 text-sm text-slate-700">
        {items.map((item) => (
          <div className="flex gap-2" key={item}>
            <CheckCircle2 className="mt-0.5 shrink-0 text-[var(--primary-green)]" size={16} />
            <span>{item}</span>
          </div>
        ))}
      </div>

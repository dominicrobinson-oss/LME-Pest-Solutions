...
import { areas, business, homeMaintenanceServices, services } from "@/lib/data";
...
export function SiteFooter() {
  return (
    <footer className="bg-[#04101A] pb-20 pt-14 text-white md:pb-10">
      <div className="container-lme grid gap-10 md:grid-cols-[1.1fr_0.9fr_0.9fr_0.9fr_0.9fr_1fr_1fr]">
        <div className="space-y-4">
          <Image
            ...
            width={132}
            height={140}
            className="h-24 w-auto object-contain drop-shadow-[0_6px_18px_rgba(200,162,74,0.25)]"
          />
          <p className="text-sm text-slate-300">Professional pest control enquiries across Manchester and North West England.</p>
          <div className="flex gap-3 text-slate-300" aria-label="Social links configurable in settings"><Circle size={18} /><Circle size={18} /></div>
        </div>
        <div>
        ...
        <div>
          <h3 className="mb-3 font-black">Home Maintenance</h3>
          <div className="grid gap-2 text-sm text-slate-300">
            {homeMaintenanceServices.slice(0, 7).map((service) => <Link href={`/home-maintenance/${service.slug}`} key={service.slug}>{service.name}</Link>)}
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="grid gap-3 text-sm text-slate-300">
            <a className="flex gap-2" href={`tel:${business.phone.replace(/\s/g, "")}`}><Phone size={16} className="text-[var(--primary-gold)]" /> {business.phone}</a>
            <a className="flex gap-2" href={`mailto:${business.email}`}><Mail size={16} className="text-[var(--primary-gold)]" /> {business.email}</a>
            <span className="flex gap-2"><MapPin size={16} className="text-[var(--primary-gold)]" /> Manchester, North West</span>
            <span>{business.openingHours}</span>
          </div>
        </div>
        <div>
          <h3 className="mb-3 font-black">Accredited & Insured</h3>
          <TrustBadges dark />
        </div>
      </div>
      <div className="container-lme mt-10 grid gap-2 border-t border-white/10 pt-6 text-sm text-slate-400">
        <p>Copyright {new Date().getFullYear()} {env.BUSINESS_LEGAL_NAME || business.name}.</p>

...
import { z } from "zod";
import type { JobStatus, LeadStatus, PublishStatus, QuoteStatus, ServiceCategory, UserRole } from "@/generated/prisma/client";
...
import { emailProvider, storageProvider } from "@/lib/providers";
...
function publishStatus(formData: FormData, fallback: PublishStatus): PublishStatus {
  const raw = value(formData, "status");
  return (publishStatuses as string[]).includes(raw) ? (raw as PublishStatus) : fallback;
}
...
export async function convertLeadToCustomer(formData: FormData) {
  ...
  await audit("LEAD_CONVERTED_TO_CUSTOMER", "Lead", { userId: user.id, entityId: leadId, newValue: { customerId: customer.id, propertyId: property.id } });
...
export async function createProperty(formData: FormData) {
  ...
  const property = await prisma.property.create({
    data: {
      ...
      postcode: postcodeSchema.parse(value(formData, "postcode")),
...
export async function sendQuote(formData: FormData) {
  ...
  if (recipient) await emailProvider.send({ to: recipient, ...emailTemplates.quoteSent(quote.quoteNumber) });
...
export async function updateQuoteStatus(formData: FormData) {
  const user = await requireAnyRole(adminRoles);
  const id = value(formData, "id");
  const status = value(formData, "status") as QuoteStatus;
  const previous = await prisma.quote.findUniqueOrThrow({ where: { id } });
  const data: Record<string, Date | string | null> = { status };
  if (status === "ACCEPTED") data.acceptedAt = new Date();
  if (status === "DECLINED") {
    data.declinedAt = new Date();
    data.rejectionReason = value(formData, "reason") || null;
  }
  const quote = await prisma.quote.update({ where: { id }, data });
  await audit("QUOTE_STATUS_CHANGED", "Quote", { userId: user.id, entityId: id, previousValue: previous, newValue: quote });
  revalidatePath(`/admin/quotes/${id}`);
}
...
export async function createInvoiceFromJob(formData: FormData) {
  ...
  if (customer?.email) await emailProvider.send({ to: customer.email, ...emailTemplates.invoiceIssued(invoice.invoiceNumber, Number(invoice.total)) });
...
export async function inviteStaff(formData: FormData) {
  const actor = await requireAnyRole(adminRoles);
  const email = z.string().email().parse(value(formData, "email").toLowerCase());
  const name = z.string().min(2).parse(value(formData, "name"));
  const role = value(formData, "role") as UserRole;
  if (role !== "SUPER_ADMIN" && !adminRoles.includes(role)) throw new Error("Invalid staff role.");
  ...
  await emailProvider.send({ to: email, ...emailTemplates.staffInvite(url) });
  await audit("STAFF_INVITED", "User", { userId: actor.id, entityId: user.id, metadata: { email, role } });
  revalidatePath("/admin/settings");
}
...
export async function saveContentPage(formData: FormData) {
  ...
  const data = {
    ...
    status: publishStatus(formData, "DRAFT"),
  };
  const page = await prisma.contentPage.upsert({ where: { slug }, update: data, create: { slug, ...data } });
...
export async function saveServiceContent(formData: FormData) {
  const user = await requireAnyRole(adminRoles);
  const slug = z.string().min(2).parse(value(formData, "slug"));
  const category: ServiceCategory = value(formData, "category") === "HOME_MAINTENANCE" ? "HOME_MAINTENANCE" : "PEST_CONTROL";
  const data = {
    name: z.string().min(2).parse(value(formData, "name")),
    category,
    ...
    status: publishStatus(formData, "PUBLISHED"),
  };
  const service = await prisma.service.upsert({ where: { slug }, update: data, create: { slug, ...data } });
  ...
  revalidatePath(`/home-maintenance/${slug}`);
}
...
export async function saveLocationContent(formData: FormData) {
  ...
  const location = await prisma.locationPage.upsert({ where: { slug }, update: data, create: { slug, ...data } });
...
export async function createCommercialContract(formData: FormData) {
  ...
  const contract = await prisma.commercialContract.create({
    data: {
      ...
      visitIntervalWeeks: intOrNull(formData, "visitIntervalWeeks"),
      visitDurationMinutes: intOrNull(formData, "visitDurationMinutes"),
      invoiceIntervalMonths: intOrNull(formData, "invoiceIntervalMonths"),
...
function contractVisitJobData(
  contract: { customerId: string; contractNumber: string; includedServices: string | null },
...
export async function createContractVisit(formData: FormData) {
  ...
  const job = await prisma.job.create({
    data: {
      jobNumber: await nextNumber("job"),
      ...contractVisitJobData(contract, property?.id, formData),
...
export async function bulkCreateContractVisits(formData: FormData) {
  ...
  const contractId = value(formData, "contractId");
  const contract = await prisma.commercialContract.findUniqueOrThrow({ where: { id: contractId }, include: { customer: { include: { properties: true } }, sites: true } });
  const intervalWeeks = Math.min(Math.max(intOrNull(formData, "intervalWeeks") || contract.visitIntervalWeeks || 4, 1), 52);
  const durationMinutes = Math.min(Math.max(intOrNull(formData, "durationMinutes") || contract.visitDurationMinutes || 60, 15), 480);
  let firstStart: Date;
  if (value(formData, "firstStart")) {
    firstStart = new Date(value(formData, "firstStart"));
  } else {
    const lastVisit = await prisma.job.findFirst({ where: { recurringJobRef: contract.contractNumber }, orderBy: { scheduledStart: "desc" } });
    firstStart = lastVisit?.scheduledStart ? new Date(lastVisit.scheduledStart.getTime() + intervalWeeks * 7 * 86_400_000) : contract.startDate || new Date();
  }
  let count = Math.min(Math.max(intOrNull(formData, "count") || 6, 1), 52);
  if (contract.endDate) {
    const maxByEndDate = Math.floor((contract.endDate.getTime() - firstStart.getTime()) / (intervalWeeks * 7 * 86_400_000)) + 1;
    count = Math.max(0, Math.min(count, maxByEndDate));
  }
  const property = contract.customer.properties[0];
  const jobCreates = [];
  ...
  const jobs = jobCreates.length ? await prisma.$transaction(jobCreates) : [];
  if (jobs.length) {
...
export async function createContractInvoice(formData: FormData) {
  ...
  const dueDate = value(formData, "dueDate") ? new Date(value(formData, "dueDate")) : null;
  const invoice = await prisma.invoice.create({
    data: {
      invoiceNumber: await nextNumber("invoice"),
      ...contractInvoiceData(contract, amount, dueDate, `Recurring contract invoice for ${contract.contractNumber}`, formData),
    },
  });
...
export async function bulkCreateContractInvoices(formData: FormData) {
  ...
  const contractId = value(formData, "contractId");
  const contract = await prisma.commercialContract.findUniqueOrThrow({ where: { id: contractId } });
  const intervalMonths = Math.min(Math.max(intOrNull(formData, "intervalMonths") || contract.invoiceIntervalMonths || 1, 1), 12);
  const amount = money(formData, "amount");
  const invoiceAmount = amount || Number(contract.contractValue);
  let firstDueDate: Date;
  if (value(formData, "firstDueDate")) {
    firstDueDate = new Date(value(formData, "firstDueDate"));
  } else {
    const lastInvoice = await prisma.invoice.findFirst({ where: { contractId: contract.id }, orderBy: { dueDate: "desc" } });
    if (lastInvoice?.dueDate) {
      firstDueDate = new Date(lastInvoice.dueDate);
      firstDueDate.setMonth(firstDueDate.getMonth() + intervalMonths);
    } else {
      firstDueDate = contract.startDate || new Date();
    }
  }
  let count = Math.min(Math.max(intOrNull(formData, "count") || 3, 1), 24);
  if (contract.endDate) {
    const monthsUntilEnd = (contract.endDate.getFullYear() - firstDueDate.getFullYear()) * 12 + (contract.endDate.getMonth() - firstDueDate.getMonth());
    const maxByEndDate = Math.floor(monthsUntilEnd / intervalMonths) + 1;
    count = Math.max(0, Math.min(count, maxByEndDate));
  }
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    invoiceCreates.push(
      prisma.invoice.create({
        data: {
          invoiceNumber: await nextNumber("invoice"),
          ...contractInvoiceData(contract, invoiceAmount, dueDate, `Bulk recurring contract invoice for ${contract.contractNumber}`, formData),
        },
      }),
    );
  }
  const invoices = invoiceCreates.length ? await prisma.$transaction(invoiceCreates) : [];
  if (invoices.length) {

...
import { adviceArticles, areas, homeMaintenanceServices, services } from "@/lib/data";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const staticRoutes = ["", "/services", "/home-maintenance", "/areas-we-cover", "/domestic", "/commercial", "/emergency", "/advice", "/about-us", "/reviews", "/faq", "/get-a-quote", "/contact", "/privacy-policy", "/cookie-policy", "/terms-and-conditions", "/complaints-policy", "/sitemap"];
  try {
    ...
    return [
      ...
      ...homeMaintenanceServices.map((service) => ({ url: absoluteUrl(`/home-maintenance/${service.slug}`) })),
